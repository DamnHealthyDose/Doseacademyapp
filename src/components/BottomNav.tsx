...
const BottomNav = () => {
  ...
  const items = [
    ...
    { to: '/spark', icon: Zap, label: 'SPARK', highlight: true },
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-bg-card border-t border-border">
      <div className="max-w-[480px] mx-auto flex items-center justify-around h-16">
        {items.map(({ to, icon: Icon, label, highlight }) => {
          const active = pathname === to || (to === '/spark' && pathname.startsWith('/spark'));
          return (
            <Link
              key={to}
              to={to}
              className={`flex flex-col items-center gap-1 px-4 py-2 rounded-button transition-all duration-200
